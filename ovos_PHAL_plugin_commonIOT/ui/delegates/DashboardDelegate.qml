...
ItemDelegate {
    ...
    onClicked: {
        if(dashboardRoot.useGroupDisplay) {
            Mycroft.MycroftController.sendRequest("ovos.iot.show.area.dashboard", {"area": deviceType})
        } else {
            Mycroft.MycroftController.sendRequest("ovos.iot.show.device.dashboard", {"device_type": deviceType})
        }
