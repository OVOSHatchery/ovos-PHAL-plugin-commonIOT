...
Mycroft.Delegate {
            ...
            if(!dashboardRoot.useWebsocket){
                ...
                if(dev_type != "main") {
                    if (dashboardRoot.useGroupDisplay) {
                        Mycroft.MycroftController.sendRequest("ovos.iot.update.area.dashboard", {"area_type": dev_type})
                    } else {
                        Mycroft.MycroftController.sendRequest("ovos.iot.update.device.dashboard", {"device_type": dev_type})
                    }
                }
            }
        }
    ...
    onGuiEvent: {
        switch (eventName) {
        case "ovos.iot.change.dashboard":
            ...
            break
        case "ovos.iot.integration.query_media.result":
            deviceControlsLoader.mediaModel = data.results
            break
        case "ovos.iot.oauth.qr.update":
            dashboardRoot.qrImagePath = Qt.resolvedUrl(data.qr)
            break
        case "ovos.iot.oauth.success":
            instaceSetupPopupBox.close()
            break
        case "ovos.iot.device.updated":
            var dev_type = tabBarModel[bar.currentIndex].type
            if(dev_type != "main") {
                if (dashboardRoot.useGroupDisplay) {
                    Mycroft.MycroftController.sendRequest("ovos.iot.update.area.dashboard", {"area_type": dev_type})
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        Switch {
                            ...
                            onCheckedChanged:{
                                if(checked) {
                                    Mycroft.MycroftController.sendRequest("ovos.iot.set.group.display.settings", {"use_group_display": true})
                                } else {
                                    Mycroft.MycroftController.sendRequest("ovos.iot.set.group.display.settings", {"use_group_display": false})
                                }
                            }
                        }
                    }
                                ...
                                onClicked: {
                                    if(dashboardRoot.horizontalMode) {
                                        if(modelData.type === "main") {
                                            ...
                                            if(dashboardRoot.useGroupDisplay) {
                                                Mycroft.MycroftController.sendRequest("ovos.iot.show.area.dashboard", {"area": modelData.type})
                                            } else {
                                                Mycroft.MycroftController.sendRequest("ovos.iot.show.device.dashboard", {"device_type": modelData.type})
                                            }
                                        }
                                    }
                                }
